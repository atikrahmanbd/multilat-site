...
export const Boxes = ({ className, ...rest }: BoxesProps) => {
	...
	const cols = new Array(250).fill(1);
	...
	// Use Index Based On Grid Position Instead Of Random During Render
	...
	return (
		<div
			style={{
				transform: `translate(-40%,-60%) skewX(-48deg) skewY(14deg) scale(0.75) rotate(0deg) translateZ(0)`,
			}}
			className={cn(
				"absolute left-1/4 p-4 -top-1/4 flex -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] z-0",
				className
			)}
			{...rest}
		>
			{rows.map((_, i) => (
				...
				>
					{cols.map((_, j) => (
						<motion.div
							whileHover={{
								backgroundColor: getColorForPosition(i, j),
